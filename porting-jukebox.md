...
Now that you are all setup, we are going to start coding. 

If you choose to add the model using `transformers-cli add-new-model-like` since the upsampers of jukebox are decoders, you would get the following : 
This command is more up to date, and should you require a processor, it will be more convenient. This is the case for Jukebox. Moreover, as it is an autoregressive model, it is pretty similar to GPT2. Jukebox uses a decoder to upsample the audio token. 
...
In our example, since we are using Jukebox, the tokenizers needs to process the genres, the author, the lyrics and the raw audio. The raw audio would have to first be `pre-processed` using the `Processor`'s `FeatureExtractor` class in order to normalise and truncate the audio for training. It is interesting to notice that in Jukebox, the actual sampling happens on the transformer, while the VQ-VAE only serves as an encoder/decoder. This is why the VQ-VAE will be a tokenizer.  
...
This article is an attempt to document how [Jukebox music generation framework](https://github.com/openai/jukebox) was ported to [`transformers`](https://github.com/huggingface/transformers/).
